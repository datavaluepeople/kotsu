...
# Validations can also take a Model _and_ an artefacts directory via kwarg
# `artefact_store_directory` which is a string, which Validations may use as the location to write
# any artefacts/data objects formed by the Validation.
# For storing Model artefacts (e.g. saving model state, or saving training history), Validations
# should pass the `artefact_store_directory` to Models directly within the Validation, and Models
# should be implemented such to use this kwarg as their output artefact store location.
